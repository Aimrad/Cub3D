...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/26 17:34:53 by artheon           #+#    #+#             */
/*   Updated: 2025/02/28 17:13:18 by artheon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	rdr->tex_index = get_texture_index(rdr->side, rdr->ray_dir_x, \
		rdr->ray_dir_y);
	...
	rdr->curr_data = rdr->tex->data[rdr->tex->current_frame];
	...
	* rdr->step;
	...
	while (rdr->y < rdr->draw_end)
	{
		...
		rdr->color = rdr->curr_data[rdr->tex->height * rdr->tex_y + rdr->tex_x];
